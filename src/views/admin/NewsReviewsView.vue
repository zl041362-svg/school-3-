...
async function handleReview(row, approved) {
  ...
  if (!approved) {
    ...
    try {
      await ElMessageBox.confirm('确认通过该资讯审核？', '审核通过', { type: 'warning', confirmButtonText: '通过', cancelButtonText: '取消' })
    } catch {
      return
    }
  }
...
<template>
  <PageContainer title="资讯审核">
    ...
    <el-table v-loading="moderationStore.loadingMap.newsReviews" :data="rows" border>
