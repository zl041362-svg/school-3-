...
| **消费者** | 有购买需求的终端用户 | 选购农产品、下单支付、收货评价、收藏资讯 |
| **农户/经营者** | 农产品生产者或经营者 | 提交身份认证、发布商品和资讯、管理订单和发货 |
...
- **双向信任**：消费者购买后可对商品进行评价，评价公开展示辅助购买决策
...
农户注册（角色为"消费者"）→ 提交身份认证（姓名、身份证、统一社会信用代码）→ 管理员审核通过（角色升级为"农户"）→ 农户发布商品（状态为"待审核"）→ 管理员审核商品（通过则"已上架"，驳回则"已驳回"）→ 商品公开可见。

### 2.2 购买发货流程

消费者浏览商品 → 加入购物车 → 填写收货信息 → 提交订单（系统校验库存、扣减库存、创建订单、清空购物车）→ 订单状态为"待发货" → 农户查看关联订单（通过商品→订单明细→订单三层关联查询）→ 农户填写物流单号发货（状态变为"已发货"）→ 消费者确认收货（状态变为"已完成"）。

### 2.3 资讯流转流程

农户发布资讯（状态为"待审核"）→ 管理员审核通过（状态变为"已发布"并记录发布时间）→ 消费者浏览资讯 → 消费者收藏/取消收藏资讯 → 个人中心查看收藏列表。
...
启动后 Flyway 自动建表并写入种子数据（3 个用户、13 个商品、12 条新闻、6 条评价）。
...
| 农户 | 13600000000 | 12345678 | 需先通过身份认证升级角色 |
...
| 个人中心 | 编辑个人资料、收货地址 CRUD + 默认地址、我的评价、我的收藏、最近浏览 |
...
| 身份认证 | 提交认证申请（姓名、身份证号、统一社会信用代码）、查看认证状态 |
...
| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/api/merchant/verify` | 提交农户认证申请 |
...
| 版本 | 内容 |
|------|------|
| V1 | 初始化 users / products / news 基础三表 |
| V2 | 种子数据：3 用户 + 3 商品 + 2 新闻 |
| V3 | 交易和管理：9 张表（交易、审核、RBAC、日志） |
| V4-V6 | 字段扩展：category、user_id、spec、qualification、状态值统一 |
| V7 | 种子数据汉化 + 新增 10 商品 + 10 新闻 |
| V8-V10 | 数据修复：密码哈希、用户名称、认证关联 |
| V11 | 外键约束：9 条 ALTER TABLE ADD CONSTRAINT |
| V12-V14 | 新功能表：favorites + product_evaluations + 种子评价 |
